...
RTMPMetadata metaDataforclients;
...
static int SetChunkSizeC3(RTMP *r)
{
	RTMPPacket packet;
	char pbuf[512]={0}, *pend = pbuf+sizeof(pbuf);
	char *enc;
...
static int SetChunkSizeP1(RTMP *r)
{
	RTMPPacket packet;
	char pbuf[512]={0}, *pend = pbuf+sizeof(pbuf);
	char *enc;
...
static int SetChunkSizeP2(RTMP*r)
{
	RTMPPacket packet;
	char pbuf[512]={0}, *pend = pbuf+sizeof(pbuf);
	char *enc;
...
static void *SendFlvFile(void*lparam)
{
	...
	{
		RTMP264_Send(&control.cam,lparam,&Nalulist,&metaDataforclients);
	}
	else
	{
		putdata(&Nalulist,lparam,&metaDataforclients);
	}
	return (void *)0;
}
...
int ServeInvoke(STREAMING_SERVER *server, RTMP * r, RTMPPacket *packet, unsigned int offset)
{
  ...
  if (AVMATCH(&method, &av_connect))
  ...
  else if (AVMATCH(&method, &av_play))
    {
      ...
      STR2AVAL(r->Link.playpath,"test.flv");
	  ThreadCreate(SendFlvFile,r);
    }
  AMF_Reset(&obj);
  return ret;
}
...
static void *Server(void *paras)//STREAMING_SERVER * server,	int sockfd	 )
{
	CLIENT_SERVER *test=(CLIENT_SERVER *)paras;
	STREAMING_SERVER * server=test->server;
...
TFTYPE acceptThread(void *arg)
{
	CLIENT_SERVER paras;
	STREAMING_SERVER *server = ( STREAMING_SERVER *)arg;
	server->state = STREAMING_ACCEPTING;
	control.client_num=0;
...
STREAMING_SERVER *startServer(const char *address, int port)
{
  ...
  ThreadCreate(acceptThread, server);
