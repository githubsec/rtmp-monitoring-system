...
struct opengles_display* ogl_display_new() {
	...
	if (result == 0) {
		LOGE("Could not allocate OpenGL display structure\n");
		return 0;
	}
...
void freeYUV(YuvBuf * buff){
	av_free(buff->planes[0]);
	av_free(buff);
}
void ogl_display_free(struct opengles_display* gldisp) {
	...
	if (!gldisp) {
		LOGE("%s called with null struct opengles_display", __FUNCTION__);
		return;
	}
	
	for(i=0; i<MAX_IMAGE; i++) {
		if (gldisp->yuv[i]) {
			freeYUV(gldisp->yuv[i]);
			gldisp->yuv[i] = NULL;
		}
	}
...
void ogl_display_set_size(struct opengles_display* gldisp, int width, int height) {
	...
	LOGV("resize opengles_display (%d x %d, gl initialized:%d)", width, height, gldisp->glResourcesInitialized);
...
void ogl_display_init(struct opengles_display* gldisp, int width, int height) {
	...
	LOGV("init opengles_display (%d x %d, gl initialized:%d)", width, height, gldisp->glResourcesInitialized);
	...
	if (!version_displayed) {
		...
		LOGV("OpenGL version string: %s", glGetString(GL_VERSION));
		LOGV("OpenGL extensions: %s",glGetString(GL_EXTENSIONS));
		LOGV("OpenGL vendor: %s", glGetString(GL_VENDOR));
		LOGV("OpenGL renderer: %s", glGetString(GL_RENDERER));
		LOGV("OpenGL version: %s", glGetString(GL_VERSION));
		LOGV("OpenGL GLSL version: %s", glGetString(GL_SHADING_LANGUAGE_VERSION));
	}
...
void ogl_display_uninit(struct opengles_display* gldisp, bool_t freeGLresources) {
	...
	LOGV("uninit opengles_display (gl initialized:%d)\n", gldisp->glResourcesInitialized);
	/*for(i=0; i<MAX_IMAGE; i++) {
		if (gldisp->yuv[i]) {
			freeYUV(gldisp->yuv[i]);
			gldisp->yuv[i] = NULL;
...
static void ogl_display_set_yuv(struct opengles_display* gldisp, YuvBuf *yuv, enum ImageType type) {
	if (!gldisp) {
		LOGE("%s called with null struct opengles_display", __FUNCTION__);
		return;
	}
...
void ogl_display_set_yuv_to_display(struct opengles_display* gldisp, YuvBuf *yuv) {
	ogl_display_set_yuv(gldisp, yuv, REMOTE_IMAGE);
}

void ogl_display_set_preview_yuv_to_display(struct opengles_display* gldisp, YuvBuf *yuv) {
...
static void check_GL_errors(const char* context) {
	...
	while (((error = glGetError()) != GL_NO_ERROR) && maxIterations > 0)
	{
		switch(error)
		{
			case GL_INVALID_ENUM:  LOGE("[%2d]GL error: '%s' -> GL_INVALID_ENUM\n", maxIterations, context); break;
			case GL_INVALID_VALUE: LOGE("[%2d]GL error: '%s' -> GL_INVALID_VALUE\n", maxIterations, context); break;
			case GL_INVALID_OPERATION: LOGE("[%2d]GL error: '%s' -> GL_INVALID_OPERATION\n", maxIterations, context); break;
			case GL_OUT_OF_MEMORY: LOGE("[%2d]GL error: '%s' -> GL_OUT_OF_MEMORY\n", maxIterations, context); break;
			case GL_INVALID_FRAMEBUFFER_OPERATION: LOGE("[%2d]GL error: '%s' -> GL_INVALID_FRAMEBUFFER_OPERATION\n", maxIterations, context); break;
			default:
...
static void allocate_gl_textures(struct opengles_display* gldisp, int w, int h, enum ImageType type) {
	...
	LOGV("%s: allocated new textures[%d] (%d x %d)\n", __FUNCTION__, type, w, h);
...
static bool_t update_textures_with_yuv(struct opengles_display* gldisp, enum ImageType type) {
	...
	if (gldisp->yuv[type]->w == 0 || gldisp->yuv[type]->h == 0) {
		LOGE("Incoherent image size: %dx%d\n", gldisp->yuv[type]->w, gldisp->yuv[type]->h);
		return FALSE;
	}
	aligned_yuv_w = align_on_power_of_2(gldisp->yuv[type]->w);
	aligned_yuv_h = align_on_power_of_2(gldisp->yuv[type]->h);
	...
	gldisp->uvx[type] = gldisp->yuv[type]->w / (float)(gldisp->allocatedTexturesSize[type].width+1);
	gldisp->uvy[type] = gldisp->yuv[type]->h / (float)(gldisp->allocatedTexturesSize[type].height+1);

	/* upload Y plane */
	GL_OPERATION(glActiveTexture(GL_TEXTURE0))
	GL_OPERATION(glBindTexture(GL_TEXTURE_2D, gldisp->textures[gldisp->texture_index][type][Y]))
	GL_OPERATION(glTexSubImage2D(GL_TEXTURE_2D, 0,
			0, 0, gldisp->yuv[type]->w, gldisp->yuv[type]->h,
			GL_LUMINANCE, GL_UNSIGNED_BYTE, gldisp->yuv[type]->planes[Y]))
	GL_OPERATION(glUniform1i(gldisp->uniforms[UNIFORM_TEXTURE_Y], 0))
	...
	GL_OPERATION(glBindTexture(GL_TEXTURE_2D, gldisp->textures[gldisp->texture_index][type][U]))
	GL_OPERATION(glTexSubImage2D(GL_TEXTURE_2D, 0,
			0, 0, gldisp->yuv[type]->w >> 1, gldisp->yuv[type]->h >> 1,
			GL_LUMINANCE, GL_UNSIGNED_BYTE, gldisp->yuv[type]->planes[U]))
	GL_OPERATION(glUniform1i(gldisp->uniforms[UNIFORM_TEXTURE_U], 1))
	...
	GL_OPERATION(glTexSubImage2D(GL_TEXTURE_2D, 0,
			0, 0, gldisp->yuv[type]->w >> 1, gldisp->yuv[type]->h >> 1,
			GL_LUMINANCE, GL_UNSIGNED_BYTE, gldisp->yuv[type]->planes[V]))
	GL_OPERATION(glUniform1i(gldisp->uniforms[UNIFORM_TEXTURE_V], 2))

	gldisp->yuv_size[type].width = gldisp->yuv[type]->w;
	gldisp->yuv_size[type].height = gldisp->yuv[type]->h;
...
#ifdef ANDROID
JNIEXPORT void JNICALL Java_com_example_h264dec_display_OpenGLESDisplay_init(JNIEnv * env, jobject obj, jint ptr, jint width, jint height) {
	struct opengles_display* d = (struct opengles_display*) ptr;
	ogl_display_init(d, width, height);
}

JNIEXPORT void JNICALL Java_com_example_h264dec_display_OpenGLESDisplay_render(JNIEnv * env, jobject obj, jint ptr) {
	struct opengles_display* d = (struct opengles_display*) ptr;
